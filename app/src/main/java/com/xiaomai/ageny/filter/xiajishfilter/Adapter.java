...
public class Adapter extends BaseQuickAdapter<String, BaseViewHolder> {
    ...
    public Adapter(int layoutResId, @Nullable List<String> data) {
        ...
        String price = SharedPreferencesUtil.getInstance(App.context).getSP("xiajiPrice");
        if (TextUtils.isEmpty(price)) {
            selectItem = 0;
        } else {
            selectItem = data.indexOf(price);
        }
    }

    @Override
    protected void convert(BaseViewHolder helper, String item) {
        helper.setText(R.id.name, item.equals("全部") ? "全部" : item + "元");
